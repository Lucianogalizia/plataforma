...
export default function HistoricoPérdidasPage() {
  ...
  const COLS: { key: SortKey; label: string; num?: boolean; highlight?: boolean }[] = [
    ...
    { key: "oilShortfall",   label: "Petróleo", num: true, highlight: true },
    { key: "waterShortfall", label: "Agua",      num: true },
    { key: "liquidShortfall",label: "Líquido",   num: true },
    { key: "gasShortfall",   label: "Gas",       num: true },
    { key: "potentialOil",   label: "Pot. Pet.", num: true },
    { key: "potentialLiquid",label: "Pot. Liq.", num: true },
  ];
                ...
                <tr>
                  {COLS.map(col => (
                    <th key={String(col.key)} onClick={() => handleSort(col.key)}
                      className="px-2 py-2 text-xs font-semibold text-slate-400 uppercase tracking-wide cursor-pointer hover:text-slate-200 select-none border-b border-[#334155]">
                      <div className={`flex items-center gap-1 ${col.num ? "justify-end" : ""}`}>
              ...
              <tbody>
                {rowsOrdenados.map((row, i) => (
                  <tr key={i} className={`border-b border-[#1a2535] hover:bg-[#1e293b] transition-colors ${i % 2 === 0 ? "bg-[#0f172a]" : "bg-[#0d1526]"}`}>
                    <td className="px-2 py-1.5 font-mono text-xs text-slate-300 truncate" title={row.POZO ?? ""}>{row.POZO ?? "—"}</td>
                    <td className="px-2 py-1.5 text-xs text-slate-400 truncate" title={row.RUBRO ?? ""}>{row.RUBRO ?? "—"}</td>
                    <td className="px-2 py-1.5 text-xs text-slate-400 truncate">{fmtDate(row["FECHA DESDE"])}</td>
                    <td className="px-2 py-1.5 text-xs text-slate-400 truncate">{fmtDate(row["FECHA HASTA"])}</td>
                    <td className="px-2 py-1.5 text-xs text-sky-300 text-right font-mono">{fmtNum(row.oilShortfall)}</td>
                    <td className="px-2 py-1.5 text-xs text-slate-400 text-right font-mono">{fmtNum(row.waterShortfall)}</td>
                    <td className="px-2 py-1.5 text-xs text-slate-400 text-right font-mono">{fmtNum(row.liquidShortfall)}</td>
                    <td className="px-2 py-1.5 text-xs text-slate-400 text-right font-mono">{fmtNum(row.gasShortfall)}</td>
                    <td className="px-2 py-1.5 text-xs text-slate-400 text-right font-mono">{fmtNum(row.potentialOil)}</td>
                    <td className="px-2 py-1.5 text-xs text-slate-400 text-right font-mono">{fmtNum(row.potentialLiquid)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
