...
async def merma_info():
    ...
    try:
        ...
        if not blob.exists():
            return JSONResponse(content={"exists": False, "updated_at": None, "file": _merma_blob_name(), "size_kb": 0})
        blob.reload()
...
async def downtimes_info():
    ...
    try:
        ...
        if not blob.exists():
            return JSONResponse(content={"exists": False, "updated_at": None, "file": _downtimes_blob_name(), "size_kb": 0, "rows": 0})
        blob.reload()
...
async def downtimes_data(
    ...
    try:
        ...
        blob   = bucket.blob(_downtimes_blob_name())
        if not blob.exists():
            raise HTTPException(status_code=404, detail=f"CSV no encontrado en gs://{GCS_BUCKET}/{_downtimes_blob_name()}. Ejecutá fetch_downtimes para generarlo.")
        content = blob.download_as_bytes()
        df      = pd.read_csv(io.BytesIO(content), low_memory=False)
    except HTTPException:
        raise
